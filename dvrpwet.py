"""Dynamic Vehicle Routing Problem without eligibility traces reinforcement learning algorithm"""
...
if __name__ == '__main__':
	...
	while time <= window * cutoff:
		...
		for i in range(len(paths)):
			...
			if len(paths[i][2][paths[i][3]:])>2:
				if paths[i][3] == 0:
					tp = tspwet(location, np.concatenate(([0], paths[i][2][paths[i][3]:])))
					paths[i][2][paths[i][3]:] = tp[1:]
				else:	
					tp = tspwet(location, paths[i][2][paths[i][3]-1:])
					paths[i][2][paths[i][3]:] = tp[1:]
	...
	if time > window * cutoff:
		...
		unserved_req = tspwet(location, np.concatenate(([0], unserved_req)))
		...
		for i in range(len(paths1)):
			if len(paths1[i][0])>2 :
				tp = tspwet(location, np.concatenate(([0], paths1[i][0])))
				paths1[i][0] = tp[1:]
	...
	for i in range(len(paths1)):
		if len(paths1[i]) and len(paths1[i][0])>2:
			paths1[i][0] = tspwet(location, np.concatenate(([0], paths1[i][0])))
